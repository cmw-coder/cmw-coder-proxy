...
namespace {
    const unordered_map<SiVersion::Major, unordered_map<Key, int>> keyMaps = {
            {SiVersion::Major::V35,
                    {
    ...
    const unordered_map<SiVersion::Major, unordered_map<Modifier, int>> modifierMaps = {
            {SiVersion::Major::V35,
                    {
    ...
    const unordered_map<SiVersion::Major, int> keyMasks = {
...
KeyHelper::KeyHelper(SiVersion::Major siVersion) noexcept:
        ...
        _printableRange(siVersion == SiVersion::Major::V35 ? make_pair(0x0020, 0x007E) : make_pair(0x000020, 0x00007E)),
        _keyMap(keyMaps.at(siVersion)),
        _modifierMap(modifierMaps.at(siVersion)) {}

optional<pair<Key, KeyHelper::ModifierSet>> KeyHelper::fromKeycode(int keycode) const noexcept {
    for (const auto &[key, keyValue]: _keyMap) {
        ModifierSet modifiers{};
        int modifierAccumulated{};
...
int KeyHelper::toKeycode(types::Key key, types::Modifier modifier) const noexcept {
    return _keyMap.at(key) + _modifierMap.at(modifier);
}

int KeyHelper::toKeycode(types::Key key, const std::unordered_set<types::Modifier> &modifiers) const noexcept {
