                ...
                switch (key) {
                    ...
                    case Key::F12: {
                        ...
                        if (fileHandle) {
                            ...
                            int symbolHandle;
                            int symbolSize;
                            getSymbolLocationFromLn(fileHandle, 44, (int)&symbolHandle, (int)&symbolSize, 0);
                            // param1: getSymbolLocationFromLn的symbolHandle
                            // param2: getSymbolLocationFromLn的symbolSize
                            // description: 处理symbolHandle的
                            ...
                            auto result = praseSymbolLocation((int*)payload2.data(), (int)hsymbol, (int)str);
                            if (!result) {
                                break;
                            }
                            //
                            ...
                            //description: 获取到childrenSymbol中的SymListCount保存到symList中
                            auto SymbolChildren = StdCallFunction<int(int, int, int, int, int*)>(
                                _baseAddress + 0x175050
                            );
                            SymbolChildren((int)hsymbol, (int)symList, 0, 0, (int*)z);
                            ...
                            //---------------SymListItem----------------------------
                            char Item[4096];
                            const auto initSymbolItemAddress = _baseAddress + 0x6F1F0;
                            // description:初始化Item
                            __asm{
                                lea ecx, Item
                                call initSymbolItemAddress
                            }
                            // param1: Item 地址
                            // param2: symList根据索引值获取到的Item信息
                            const auto getSymbolItemAddress = _baseAddress + 0x68910;
                            // 10是索引值
                            const auto symListItem = *(symList+2) * 10 + *(symList+5) + 0x0C;
                            __asm{
                                push symListItem
                                lea ecx, Item
                                call getSymbolItemAddress
                            }

                            CompactString childPayload;
                            getSymbolFormat(Item, (int)childPayload.data());
                            logger::debug("1");
